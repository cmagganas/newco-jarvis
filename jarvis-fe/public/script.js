...
function sendToServer(transcript) {
    fetch('https://theintersection.com/jarvis', {
        method: 'POST',
        headers: {
...
function speakText(text) {
    ...
    if (text === "Action") {
        ...
        let audio = new Audio('/jarvis-fe/sounds/I loaded the data from the plex.mp3');
        audio.play();
    }
    else if(text === "Order") {
        let audio = new Audio('/jarvis-fe/sounds/Minster order C 1 2 4 confirmed.mp3');
        audio.play();
    }
    else if(text === "Looking") {
        let audio = new Audio('detected.mp3');
        audio.play();
    }
}
...
const actionPhrases = ["capa", "kappa", "kapa", "papa", "order", "plex", "jarvis"];
...
document.addEventListener('DOMContentLoaded', function() {
            ...
            if (event.results[i].isFinal) {
                ...
                if (transcript === "action") {
                    ...
                    if(transcript.includes("order")) {
                        message = {
                            "message": "order 124",
                            "processed": "{\"order_id\": \"1234\"}"
                        };
                        jsonData.messages.push(message);
                        updateJsonOutput();
                        speakText("Order");
                    }
                    else if(transcript.includes("plex")) {
                        message = {
                            "Minster": {
                            "Machine Specifications": {
                              "Model": "Minster P2-150",
    ...
    function sendToServer(transcript) {
        fetch('https://theintersecto.com/jarvis', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
